...
export const HeatmapWidget: React.FC<Props> = ({ heatmapData }) => {
  ...
  const filtered = heatmapData.filter(
    item => selectedSector === 'All' || item.sector === selectedSector
  );

  // Get background color based on daily percentage performance
    ...
    <div className="flex flex-col h-full bg-slate-950/80 text-slate-200 select-none overflow-hidden relative">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 p-2.5 bg-slate-900/90 border-b border-slate-800 text-xs">
        <div className="flex items-center gap-1.5">
          ...
          {(['All', 'AI & Tech', 'Energy', 'Financials'] as const).map(sec => (
            <button
              key={sec}
              onClick={() => setSelectedSector(sec)}
              className={`px-2 py-0.5 rounded text-[11px] font-mono transition-colors ${
                selectedSector === sec
                  ? 'bg-cyan-950 text-cyan-400 border border-cyan-700/60 font-semibold'
        ...
        <div className="flex items-center gap-1 text-[10px] font-mono text-slate-400">
          <span className="w-2.5 h-2.5 bg-rose-600 rounded-xs inline-block"></span>
          <span>-4%</span>
          <span className="w-2.5 h-2.5 bg-slate-800 rounded-xs inline-block ml-1"></span>
          <span>0%</span>
          <span className="w-2.5 h-2.5 bg-emerald-600 rounded-xs inline-block ml-1"></span>
          <span>+4%</span>
        </div>
      </div>
      ...
      <div className="flex-1 p-2 overflow-y-auto custom-scrollbar">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-1.5 auto-rows-fr h-full min-h-[220px]">
          {filtered.map(item => {
            ...
            // Dynamically scale tile size weight based on market cap
            const colSpan = item.marketCapBillions > 2000 ? 'col-span-2 row-span-2' : 'col-span-1 row-span-1';
            ...
            return (
              <div
                ...
                className={`p-2 rounded border flex flex-col justify-between transition-all duration-200 cursor-pointer hover:scale-[1.02] hover:z-10 ${bgClass} ${colSpan}`}
              >
                <div className="flex items-center justify-between w-full">
                  <span className="font-mono font-bold text-xs tracking-wider">{item.ticker}</span>
                  <span className="text-[10px] opacity-75 font-mono">${item.marketCapBillions}B</span>
                </div>

                <div className="my-1">
                  <div className="text-[11px] font-mono truncate opacity-90 font-medium">
                    {item.name}
                  </div>
                  <div className="font-mono font-bold text-sm tracking-tight mt-0.5">
                    ${item.price.toFixed(2)}
                  </div>
                </div>

                <div className="text-[11px] font-mono font-bold self-end">
      ...
      <div className="p-2 bg-slate-900 border-t border-slate-800 text-xs font-mono flex items-center justify-between text-slate-400">
        {hoveredTicker ? (
          <div className="flex items-center gap-3 text-slate-200">
            ...
            <span className="text-amber-400">MCAP: ${hoveredTicker.marketCapBillions}B</span>
            <span className={hoveredTicker.changePercent >= 0 ? 'text-emerald-400 font-bold' : 'text-rose-400 font-bold'}>
              {hoveredTicker.changePercent >= 0 ? '+' : ''}{hoveredTicker.changePercent}%
            </span>
          </div>
        ) : (
          <div className="flex items-center gap-1.5 text-slate-500">
            <Info className="w-3.5 h-3.5" />
            <span>HOVER TILE TO VIEW DETAILED SECTOR WEIGHT & MARKET CAP</span>
          </div>
        )}
      </div>
    </div>
