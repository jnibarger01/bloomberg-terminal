  ...
  <div className="flex flex-col h-full bg-slate-950/80 text-slate-200 select-none overflow-hidden font-mono">
    <div className="p-2 bg-slate-900 border-b border-slate-800 flex items-center justify-between text-xs">
      <div className="flex items-center gap-2 text-amber-400">
        ...
        <span className="font-bold text-slate-200">MARKET NEWS WIRE</span>
      </div>
      <div className="flex items-center gap-1.5 text-[10px] text-slate-400">
        <Radio className="w-3 h-3" />
        <span>PROVIDER OR SIMULATED MODE</span>
      </div>
    </div>
    ...
    <div className="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar">
      {news.map((item) => {
        ...
        return (
          <div key={item.id} className={`p-2 rounded border ${urgencyClass}`}>
            ...
            <div className="text-xs font-semibold leading-relaxed">{item.headline}</div>
          </div>
        );
      })}
      ...
      {news.length === 0 && (
        <div className="h-full min-h-[180px] flex items-center justify-center text-xs text-slate-500 text-center px-4">
          NO PROVIDER NEWS AVAILABLE. SIMULATED NEWS IS NOT USED AS A PROVIDER FALLBACK.
        </div>
      )}
    </div>
  </div>
